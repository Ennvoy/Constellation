...
**效率紀律（快不折正確性）**：正確性三道保險之外，測試的「成本」也在寫下去的當下管——①新測試寫在能抓到該 bug 的最低層（unit→整合/db→e2e），e2e 只驗跨層縫合，新增 e2e 在票的決議記錄留一句「為什麼低層測不到」；②技術形態遵守 verification-playbook「真鏈路也要快」（本地同引擎隔離庫＝真依賴、不預先建基建；少往返批次寫法；登入態一次取得重用；輕量資料隔離；追蹤產物只留失敗）；③weave 生成 journey 時同框架多 project 合併單一呼叫，昂貴固定成本一次付；④ship 全量預期超過 10 分鐘以背景執行，期間平行開兩軸審查。**耗時不設任何自動門檻**：runner 照記各指令耗時入證據當純紀錄，工作流不比對上輪、不算成長率、不自動提改善方案——要不要優化由使用者看數字自行決定。
