...
**接續**：新 session 開場由 session-start 閘門自動注入**純導航**——置頂【開工前必讀】強制讀檔指令（動手任何工作前先 Read `MAP.md` 與 `CONTEXT.md`，並寫明決議的**查法**）＋①票況（含驗證 runner 絕對路徑）②`MAP.md` **模組索引**與過期警示 ③`HISTORY.md` 最近輪次 ④`decisions/` **一行摘要**（共幾筆、最近的編號範圍、怎麼查）⑤`CONTEXT.md` **一行摘要**（共幾行幾詞條）。地圖排在票況之後——「東西在哪」是動手前最先要知道的事。
