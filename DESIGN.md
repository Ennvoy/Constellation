...
4. **定稿即凍結**：定稿元件檔案清單寫入 `.constellation/design-frozen.json`，實作期間關票刷卡機攔截任何對凍結檔案的編輯——**要改定稿 UI 必須先經使用者彈窗同意、解凍並記錄原因**（日誌留軌跡，出貨審查核對）；改完可重新凍結。「討論定版的畫面被無聲改掉」從機制上根絕。
5. **降級路徑**：Claude Design 不可用（離線／額度／Codex runtime）時退化為純本地變體走查（瀏覽器開 `?variant=` 切換），流程其餘不變。
