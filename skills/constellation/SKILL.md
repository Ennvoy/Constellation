...
| ② UI 定稿 | design | Claude Design 自己設計（或執行端自己寫）→ **直接改專案正式頁面 code** → **開本地 dev server 請使用者親自點過拍板** → 才算定稿（真數字／後端後接）|
...
2. **`.constellation/` 存在，但 `tickets/` 不存在或裡面沒有任何 `*.md`**：
   ...
   - `decisions/grill-close.md` 存在（內容為大小流程、是否需要 UI、高風險標記、必備模組排除、一句話任務摘要五欄）→ 訪談已完成、尚未拆票。
     ...
     ⚠ **「已定稿」不是看有沒有那筆定稿決議就算**——weave 進場要過機器三驗（定稿記錄含逐區塊元件清單與本地拍板紀錄／`design-frozen.json` 的 `frozen` 非空／名單每個路徑真的存在於 repo），任一條不成立就是沒定稿、要退回 design 補做。這三條純檔案檢查、兩個 runtime 都一樣，細節在 `references/phase-weave.md` 進場條件。
...
- **runtime 降級對照**（此對照放總控，是因為 lazy loading 下 Codex 在後期階段讀不到 `phase-grill.md` 裡的降級說明；各 `references/phase-*.md` 提到下列三個工具時，Codex 端一律按此對照執行，紀律不變、形式退化。**前兩條是按 runtime 分的；第三條不是**——那條看的是工具此刻在不在，兩邊都可能用不上，見該條說明）：
  ...
  - **DesignSync／Claude Design canvas** → **判斷依據是「工具此刻在不在、通不通」，不是「跑在哪個 runtime」**：Claude Code 端離線或額度用完一樣用不上，另一邊哪天掛上可用的工具、登入也通了就用得上，所以每次進 design 階段先實際確認一次。確認得出這次用不上時**不默默降級**——先用白話告知使用者落差（含那句誠實話：執行端自己寫幾個版本，那幾版只是同一個判斷的幾種寫法，不等於真的比較過不同方案），再讓他選「換到有這個服務的環境做完這一步」或「不換、由執行端自己寫畫面 code 用文字來回修」。選自己寫時預設先做一版再迭代（不是一次生 3 版），東西本來就在 repo 裡，少了「拉回」與「轉譯」兩步，定稿記錄與凍結照跑。小工程（改一個按鈕／加一個欄位／既有設計語言下的單一元件／bug fix）不必問，直接改 code 給使用者看——此分流不限降級情境，Claude Design 可用時同樣直通；細節與「大小工程怎麼分」在 `references/phase-design.md` 的「小工程直通」。**Codex 端另外沒有彈窗**，上面那個「換／不換」的提問比照本對照第一條改成文字點列。
  - **瀏覽器工具（②的步驟 5b 裡「交件前執行端自己預檢畫面」那一小步）** → **只降這一小步**：Codex 端裸 CLI **沒有內建瀏覽器**（母本 §11.5 本機實查：官方明寫 ChatGPT／Codex 的內建 Browser 不提供給 Codex CLI 與 IDE extension），改以「打 HTTP 確認頁面回 200 ＋ 關鍵區塊的標記真的出現在回傳 HTML 裡」＋既有元件 render 測試代替，並在交件那句話**誠實講明「我沒有親眼看過渲染結果，破圖請直接說」**；不得因為看不到就跳過預檢，也不得假稱看過。
    **⚠ 但「起本地 dev server ＋ 把網址交給使用者、他拿自己的瀏覽器親自點過拍板」那一步，兩邊一模一樣、不降級**——那是②唯一的定版點（跟 runtime 無關，使用者用的是他自己的瀏覽器），任何 runtime 都不得省略、不得代為拍板、不得用截圖替代。拍板的**提問形式**照本對照第一條降級（Codex 走文字點列），拍板這件事本身不打折。
