...
5. **關票**：runner 綠燈後，把票的 `status` 改 `done`。閘門 5（關票刷卡機）會檢查驗證證據是否存在且夠新鮮，證據不足直接擋下，不能手動繞過。
   - **動到模組結構就順手更新地圖**：這張票如果新增／搬移了模組或目錄、新增或刪除了資料表，就在 `.constellation/MAP.md` 更新對應那一行（母本 DESIGN.md §4）；查出了新的資料缺口或地雷也補進第三段。**只改內容、沒動結構的票不必碰它**——地圖只記路徑。唯一例外：這張票**補掉了第三段某個缺口、或拆掉了某個地雷**，那條當下就刪掉——已經不成立的缺口留著比沒寫還糟，下一輪會照著它繞路，或重做一次已經做完的事。當下順手改是幾秒的事，留到 ship 才補就得回頭翻本輪每張票做了什麼。
