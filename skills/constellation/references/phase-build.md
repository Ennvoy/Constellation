...
5. **Commit**：這張票的改動做一個 commit，訊息描述這張票做了什麼（不是逐檔羅列變更）。
6. **關票**：狀態改 `done`。閘門 5（關票刷卡機）會檢查驗證證據是否存在且夠新鮮，證據不足直接擋下，不能手動繞過。
...
- 用 Workflow 腳本 fan-out，worker 用便宜模型，各自跑上面「單票內部循環」的步驟 1～5。
