...
- **校對專案現況地圖 `.constellation/MAP.md`**（母本 DESIGN.md §4）：本輪如果新增／搬移了模組、新增或刪除了資料表，把對應行更新；同時把本輪查出來的**新的資料缺口與地雷**補進第三段（那一段是「看程式碼看不出來的不存在」，正是下一輪開工前盤點最省時間的地方）。**不必手動標記同步點**——閘門 3 直接拿「`MAP.md` 最後一次被 commit 的那個 commit」當過期基準自己算（母本 DESIGN.md §4），所以地圖跟著票一起 commit，基準就自動跟上。**地圖不存在就順手建一份**（grill 階段的開工前盤點應該已經產出，沒有代表那一步被跳過了）。這一步只花幾分鐘：地圖只記路徑不記內容，本輪沒有新增或搬移檔案時通常一行都不用改。
