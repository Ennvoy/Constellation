...
- **驗了什麼**：全量驗證跑了哪些 test／journey、結果如何；兩軸審查各自看了什麼範圍。並列出各指令耗時（取自 `ship-evidence.md` 證據筆的耗時行）、與上一筆 ship 證據對比；總耗時成長逾三成→點名最貴的指令、在報告裡建議開一張測試優化票（優化手段屬專案工程債，不寫進母本條文）。耗時是儀表不是閘門：不影響綠燈判定，也不得為壓耗時縮小 ship 驗證範圍。
