...
**方向感（這階段最容易走反的地方）**：Claude Design 的價值在於**它自己會設計**——官方定位是「你講你要什麼，它先做一版給你反應，你再一路改到它變成你的」。所以**不要**在本地先做好幾個版本再推上去讓使用者挑：那等於白繞一趟雲端，也浪費了它自己會設計這件事。自己寫畫面 code 只在「這次用不上 Claude Design」時才走（見最後的「降級路徑」）。
...
- **進場後第一件事**：先確認這次到底用不用得上 Claude Design（見步驟 1 之前的前置判斷）。用得上就照步驟 1～7 走主線；用不上就走最後的「降級路徑」。
...
**不是所有跟畫面有關的工作都需要設計服務。**只改一個按鈕、修一個小地方、加一個欄位、在既有設計語言底下做單一元件、修畫面的 bug——這類**直接改 code 給使用者看就好**（自己開本地頁面跑一次、截圖或給網址請他看），**不必問要不要換環境**；問了只是浪費使用者的時間，也違反「流程本身不得成為工作量」（出處：母本 DESIGN.md §0，部署後 runtime 不需讀取）。改完照下面「使用者選『你自己寫』時怎麼做」的第 4 點往後走（給他看、依文字回饋改、滿意即定稿、寫記錄、凍結）。

**要停下來問的是這幾種**：新產品、全新頁面、資訊架構要重新決定、複雜的操作流程。這種情況下少了 Claude Design 是真的有落差，得讓使用者知道、由他拍板。（`grill-close.md` 記著 `是（照既有架構做）` 的需求多半落在小工程那一側，但「照既有架構做的全新頁面」或複雜流程仍算大工程，照樣要問——判斷依據是這次要決定的東西有多大，不是那一欄的值。）
